# Project Exploration
## Project Title:
P5.js

## Project Repository:
https://github.com/processing/p5.js

## Project Homepage:
http://beta.p5js.org

## Product:
Web client that can develop creative art.

## Target User Community:
Coders, Artists, Designers, Developers

## Activity:
19 Branches, 14031 Commits

## Getting Started:
https://github.com/processing/p5.js/blob/main/CONTRIBUTING.md

## Welcoming to Newcomers:
https://github.com/processing/p5.js/blob/main/README.md

## Personal Interest:
I enjoy creating things

## Skills Match:
JS, HTML which I know.

## Other:
a good chunk uses Markdown, this sounds like something I could learn (possibly formatting).

## Aleena: 
I would be ok working on this because I know JavaScript and HTML.

# Project Review

## Getting Started

### Documents
https://github.com/processing/p5.js/blob/main/README.md
https://github.com/processing/p5.js/blob/main/CONTRIBUTING.md
https://github.com/processing/p5.js/blob/main/license.txt

### Code of Conduct
https://github.com/processing/p5.js/blob/main/CODE_OF_CONDUCT.md
   
### User Installation
They go to p5.js on browser.

### Developer Installation
https://p5js.org/contribute/contributor_guidelines/#quick-get-started-for-developers

### Organization
Were the documents you found easy to find? No, slightly organized
Was it clear which documents would be useful to you? Yes, super organized
Is the amount of information helpful without being overwhelming? Yes, organized
Is there out of date or redundant information that may be confusing? Yes, organized

### Summary
This project allows anyone to make realistic animations, graphics, games using JS.

## Community Communication
- [Forum](https://discourse.processing.org/c/p5js)
  - **Purpose:** Main forum for P5JS
  - **Currency:** less than 1 day
  - **Activity:** 4 messages last day in one of channels, most are split into specialized categories.
  - **Responsiveness:** likely
  - **Response Time:** Possibly within couple hours. Less frequent than discord.
  - **Content:** Usually text.
- [Discord](https://discord.gg/SHQ8dH25r9)
  - **Purpose:** Essentially another forum for P5JS
  - **Currency:** 1 day
  - **Activity:** Hard to tell as there are multiple channels but there are occasional updates on some and very frequent on others
  - **Responsiveness:** should be likely but varies on channel.
  - **Response Time:** Should be within a day.
  - **Content:** There are some about sharing work, community discussion

## Repositories
- [Repository name](https://github.com/processing/p5.js)
  - **Purpose:** Main Repository
  - **Created:** 2013-02-26
  - **License:** LGPL-2.1
- [Repository name](https://...)
  - ...

## Issue Tracking
- [Issue Tracker](https://...)
  - **Volume:** 381
  - **Currency:**
    - Last Day: 0
    - Last Week: 3
    - Last Month: 12
    - Last 6 Months: 90
  - **Activity:**
    - Last Day: 0
    - Last Week: 7
    - Last Month: 54
    - Last 6 Months: 151
  - **Beginner Issues:**
    - Labels: Good First Issue
    - Currency:
      - Last Week: 0
      - Last Month: 0
      - Last 6 Months: 7
    - Resolution: 
      - Last Week: 0
      - Last Month: 1 completed
      - Last 6 Months: 2 completed
    - Approachability: Not sure if there are open beginner issues, since all appear to be assigned to someone
    - Summary: Not sure where a person would start since all the beginner issues are taken except for two.

## Pull/Merge Requests
- **Volume:** 123
- **Currency:**
  - Last Week: 4
  - Last Month: 15
  - Last 6 Months: 62
- **Activity:**
  - Last Week: 5
  - Last Month: 23
  - Last 6 Months: 69
- **Resolution:**
  - Last Week: 6 merged, 13 not merged
  - Last Month: 90 merged, 19 not merged
  - Last 6 Months: 296 total, 220 merged, 76 not merged
- **Contributors:**
  - Last Month: 4
  - Last Year: 11
- **New Contributors:**
  - Last Month: 0
  - Last Year: 
- **Summary:**

## Tools/Languages/Libraries/Frameworks:
- **Tools:**
  - NPM
- **Languages:**
  - JS (65%), Markdown (32.5%), HTML (1.5%)
- **Application Libraries/Frameworks:**
  - Processing
- **Testing Libraries/Frameworks:**
  - Mocha
- **Summary:**
I believe this should be easy for people who know JS, since theres extensive documentation and it is actively being worked on.
   
## Assessment
I would say this is a very interesting project to work on.